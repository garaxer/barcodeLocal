...
import { generateZipOfImagesFromRefs, generateImageFromRef } from '../../util/generateBarcode';
...
const MakeBarcodes = ({values}) => {
  ...
  useEffect(() => {
    values && (values.length === 1 ? generateImageFromRef(itemsRef.current[0])
...
const mapStateToProps = (state, ownProps) => ({
  ...
  values: Object.keys(ownProps).includes('values') ? ownProps.values : state.pngBarcodes,
});
